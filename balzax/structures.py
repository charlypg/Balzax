...
def out(ball: Ball, down=0.0, up=1.0):
    """ball in the square delimitted by down and up ?"""
    is_out = jnp.any(ball.pos < 0.0)
    is_out = is_out | jnp.any(ball.pos > 1.0)
    return is_out
...
def pen_res_bw(b: Ball, w: Wall) -> Ball:
    ...
    return Ball(
        pos=b.pos - jax.nn.relu(b.radius - dist_to_closest) * diff_unit, radius=b.radius
    )
...
def pen_vect_bb(
    ...
    pen_vect = jax.nn.relu((radius_1 + radius_2) / distance - 1.0) * diff
    return pen_vect
...
def update_bb_indices(balls: Ball, index_1: int, index_2: int) -> Ball:
    ...
    new_pos_1, new_pos_2 = pen_res_bb_pos(
        balls.pos[index_1],
...
def update_agent_pos(balls: Ball, dpos: jnp.ndarray, agent_index: int = 0) -> Ball:
