...
def ball_to_imbool(reg_ball: Ball, image_dim: int = 224):
...
def ball_to_image(
    reg_ball: Ball,
    color: jnp.ndarray = jnp.array([1.0, 0, 0], dtype=jnp.float32),
    image_dim: int = 224,
):
    """Returns an image of a colored ball."""
    mask = jnp.expand_dims(ball_to_imbool(reg_ball, image_dim), axis=-1)
    return mask * color


def balls_to_images(reg_ball: Ball, color: jnp.ndarray, image_dim: int = 224):
    """Returns a batch of colored balls"""
    return jax.vmap(ball_to_image, in_axes=[0, 0, None])(reg_ball, color, image_dim)


def balls_to_one_image(reg_ball: Ball, color: jnp.ndarray, image_dim: int = 224):
    """Returns one image with all batched balls"""
    return jnp.clip(
        jnp.sum(balls_to_images(reg_ball, color, image_dim), axis=0), 0, 1.0
    )
